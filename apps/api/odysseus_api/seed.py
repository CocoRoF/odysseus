"""데모 시드 — 계정 3개 + 완성형 데모 시나리오/시험.

데모 시나리오 '주간 매출 리포트 이상'은 이 플랫폼의 문제 정의를 그대로 보여준다:
지문은 없다. PM의 메신저 한 통에서 시작해, 데이터 엔지니어에게 스키마와 집계
규칙을 캐묻고, QA의 재현 사례로 가설을 검증한 뒤, 버그난 스크립트를 고쳐
정확한 리포트 파일을 만들어야 한다.
"""
...
async def seed_if_empty(db: AsyncSession) -> None:
    ...
    scenario = Scenario(
        ...
        difficulty="medium",
        briefing_md="출근했습니다. **메신저**에 새 메시지가 와 있습니다.\n\n무엇을 해야 하는지는 아무도 문서로 주지 않습니다 — 관계자와 대화하며 직접 파악하세요.",
    ...
    db.add(assessment)
    await db.flush()
    db.add(AssessmentScenario(assessment_id=assessment.id, scenario_id=scenario.id, ordinal=0, points=100))
    db.add(Assignment(assessment_id=assessment.id, user_id=candidate.id))

    await db.commit()
    print("[seed] demo users + scenario + assessment created")
