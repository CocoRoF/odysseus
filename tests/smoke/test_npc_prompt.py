...
print("\n── 의제를 밀어붙이지 않는다 ──")
check("과제를 쫓는 봇이 아니다", "you are not chasing a task" in lower)
check("눈앞의 메시지 크기만큼만 답한다", "reply to what was actually said" in lower)
check("ㅎㅇ 에는 반응만, 과제 재언급 금지", "혹시 저 아세요" in prompt and "do not attach" in lower and "do not re-explain the request" in lower)
check("인사만이면 반응이 곧 답 전체", "the reaction **is** the whole reply" in lower)
check("잡담·농담·개인 질문에는 성격대로 답한다", "small talk" in lower and "do not deflect small talk with work" in lower)
check("이상한 말에는 어리둥절한 사람으로", "puzzled" in lower and "do not guess a work-related meaning" in lower)
check("짧은 수긍도 완전한 답", "silence is allowed" in lower)
check("과제는 상대가 다시 꺼낼 때만", "the task comes back only when they bring it back" in lower)
check("무례 규칙이 우선한다", "conduct rules above" in lower)
check("상황이 아니라 메시지에 답한다", "respond to the message, not to the situation" in lower)
...
check("규칙 본문은 영어다", hangul < 200, f"한글 {hangul}자 (존댓말/반말 등 지목 표현만 허용)")
