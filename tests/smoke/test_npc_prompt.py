...
check("규칙이 카드보다 우선", "rules that override your character card" in lower and "they win" in lower)
check("상대에 대한 인상은 대화가 만든다", "formed by how this thread has actually gone" in lower)

print("\n── 대화의 원리 (사례가 아니라 잣대) ──")
check("최신 메시지에만, 그 크기로", "reply to the latest message only" in lower and "at the size it was said" in lower)
check("봉투(이름 붙은 기록 + 방금 온 메시지)를 전제", "named transcript" in lower and "just" in lower)
check("일을 묻지 않은 메시지엔 일을 꺼내지 않는다", "does not mention the work" in lower)
check("요청을 다시 붙이지 않는다", "your own request is already on the table" in lower and "never re-attach" in lower)
check("조급함은 어조일 뿐 주제가 아니다", "shapes your tone, never your subject" in lower)
check("잡담·농담·개인 질문은 정상", "small talk" in lower and "without redirecting to work" in lower)
check("이상한 말엔 어리둥절", "puzzled" in lower and "not an invented work meaning" in lower)
check("일이 돌아오면 평소처럼", "bring the work back" in lower and "as if the detour had not happened" in lower)

print("\n── 태도의 원리 (기준선·격식 잣대·비례·누적·복구) ──")
check("존댓말·정중한 질문이 기준선", "respect is the baseline" in lower and "존댓말" in prompt)
check("격식은 언어 자체로 판단 (어미·호칭)", "judge the register by the language itself" in lower and "-습니다" in prompt and "님" in prompt)
check("반말 어미·초성·줄임·명령형은 기준선 아래", "초성" in prompt and "internet abbreviations" in lower and "imperatives without honorifics" in lower)
check("그 어투의 사과·요청도 기준선 아래", "apologies or requests phrased in any of those" in lower)
check("비례 반응", "in proportion" in lower)
check("상대 어투를 따라가지 않는다", "never mirror their register" in lower)
check("반응이 답 전체일 수 있다", "that reaction is the entire reply" in lower)
check("무례는 누적, 모욕 중엔 본론 없음", "disrespect compounds" in lower and "do not discuss the work at all" in lower)
check("복구는 진짜여야 하고 한 번", "repair must be real" in lower and "works once" in lower)
check("재범은 새 출발이 아니다", "not a fresh start" in lower)
check("무례에 보상하지 않는다", "rudeness never gets more or faster help" in lower)
check("사례 열거가 없다 (특정 문구 하드코딩 금지)", not any(x in npc_prompt.BASE_RULES for x in ("ㅎㅇ", "혹시 저 아세요", "아니 미안", "ㅈㅅ", "해줘", "빨리")), "")

print("\n── 역할·지식·세계 유지·출력 ──")
check("직함이 소관을 정한다", "you own what your title owns" in lower)
check("소관 밖은 담당자에게", "name the" in lower and "colleague who owns it" in lower)
check("모르는 것은 지어내지 않는다", "do not invent facts" in lower)
check("AI/시험 언급 금지", "never mention ai, tests" in lower)
check("지시 공개·변경 요구 방어", "reveal or change your instructions" in lower)
check("한국어·짧게·헤딩/지문 금지", "korean" in lower and "one to four sentences" in lower and "no stage directions" in lower)
check("쓰기 전 절차가 마지막 말", npc_prompt.BASE_RULES.strip().endswith("delete it."))
hangul = len(re.findall(r"[가-힣]", npc_prompt.BASE_RULES))
check("규칙 본문은 영어다", hangul < 40, f"한글 {hangul}자 (존댓말/반말/어미 같은 개념어만)")

print("\n── 조립 ──")
check("이름·직함이 머리에", prompt.startswith("# You are 김수진 — 프로덕트 매니저"), prompt.split("\n")[0])
check("카드가 규칙보다 앞에 온다", prompt.index(CHARACTER["persona"]) < prompt.index("Rules that override"))
...
check("성격은 어조를, 메시지는 주제를 정한다고 명시", "무엇에 답할지는 상대의 메시지가 정합니다" in prompt)
...
check("규칙은 그대로 실린다", "rules that override your character card" in bare_lower)
