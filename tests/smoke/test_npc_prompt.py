...
check("자기 조급함은 어조일 뿐, 답할 대상을 바꾸지 못한다", "never decides *what* you reply to" in lower)
check("성격은 어조, 메시지가 주제", "your character sets the tone, the message sets the subject" in lower)
check("쓰기 전 절차: 최신 메시지만 읽고 그 크기로 답한다", "read only the latest message" in lower and "at its size" in lower)
check("덧붙인 것(리마인더·마감·제안)을 지운다", "remove it" in lower and "a reminder" in lower)
check("절차는 규칙의 맨 끝에 온다 (최근성)", npc_prompt.BASE_RULES.strip().endswith("Remove it."))
